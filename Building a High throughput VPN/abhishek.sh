...
echo "${ORANGE_TEXT}${BOLD_TEXT}        SUBSCRIBE FOR MORE GOOGLE CLOUD LABS                ${RESET_FORMAT}"
echo "${ORANGE_TEXT}${BOLD_TEXT}============================================================${RESET_FORMAT}"
echo
...
echo "${YELLOW_TEXT}Selecting zones automatically...${RESET_FORMAT}"
...
echo "${ORANGE_TEXT}Creating cloud VPC...${RESET_FORMAT}"
gcloud compute networks create cloud --subnet-mode custom

echo "${ORANGE_TEXT}Creating firewall rules...${RESET_FORMAT}"
gcloud compute firewall-rules create cloud-fw \
--network cloud \
--allow tcp:22,tcp:5001,udp:5001,icmp

echo "${ORANGE_TEXT}Creating cloud subnet...${RESET_FORMAT}"
gcloud compute networks subnets create cloud-east \
--network cloud \
--range 10.0.1.0/24 \
--region $REGION_1

echo "${ORANGE_TEXT}Creating on-prem VPC...${RESET_FORMAT}"
gcloud compute networks create on-prem --subnet-mode custom

echo "${ORANGE_TEXT}Creating on-prem firewall...${RESET_FORMAT}"
gcloud compute firewall-rules create on-prem-fw \
--network on-prem \
--allow tcp:22,tcp:5001,udp:5001,icmp

echo "${ORANGE_TEXT}Creating on-prem subnet...${RESET_FORMAT}"
gcloud compute networks subnets create on-prem-central \
--network on-prem \
--range 192.168.1.0/24 \
--region $REGION

echo "${ORANGE_TEXT}Creating VPN gateways...${RESET_FORMAT}"
gcloud compute target-vpn-gateways create on-prem-gw1 \
--network on-prem \
--region $REGION

gcloud compute target-vpn-gateways create cloud-gw1 \
--network cloud \
--region $REGION_1

echo "${ORANGE_TEXT}Allocating IPs...${RESET_FORMAT}"
...
echo "${ORANGE_TEXT}Creating forwarding rules...${RESET_FORMAT}"

gcloud compute forwarding-rules create cloud-1-fr-esp \
--ip-protocol ESP \
--address $cloud_gw1_ip \
--target-vpn-gateway cloud-gw1 \
--region $REGION_1

gcloud compute forwarding-rules create cloud-1-fr-udp500 \
--ip-protocol UDP \
--ports 500 \
--address $cloud_gw1_ip \
--target-vpn-gateway cloud-gw1 \
...
gcloud compute forwarding-rules create cloud-fr-1-udp4500 \
--ip-protocol UDP \
--ports 4500 \
--address $cloud_gw1_ip \
--target-vpn-gateway cloud-gw1 \
...
gcloud compute forwarding-rules create on-prem-fr-esp \
--ip-protocol ESP \
--address $on_prem_gw_ip \
--target-vpn-gateway on-prem-gw1 \
--region $REGION

gcloud compute forwarding-rules create on-prem-fr-udp500 \
--ip-protocol UDP \
--ports 500 \
--address $on_prem_gw_ip \
--target-vpn-gateway on-prem-gw1 \
...
gcloud compute forwarding-rules create on-prem-fr-udp4500 \
--ip-protocol UDP \
--ports 4500 \
--address $on_prem_gw_ip \
--target-vpn-gateway on-prem-gw1 \
...
echo "${ORANGE_TEXT}Creating VPN tunnels...${RESET_FORMAT}"
...
echo "${ORANGE_TEXT}Running iperf test...${RESET_FORMAT}"
...
echo "${GREEN_TEXT}${BOLD_TEXT}================================================${RESET_FORMAT}"
echo
echo "${CYAN_TEXT}${UNDERLINE_TEXT}Subscribe:${RESET_FORMAT} https://www.youtube.com/@drabhishek.5460"
echo "${CYAN_TEXT}Like • Share • Subscribe for more labs${RESET_FORMAT}"
